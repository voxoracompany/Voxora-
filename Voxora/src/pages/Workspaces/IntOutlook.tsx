// ── V5.7 Microsoft Outlook Integration ───────────────────────────────────────
...
export default function IntOutlook({ setWorkspace }: Props) {
  const [status, setStatus] = useState(IntegrationService.getStatus("outlook"));
  ...
  const handleConnect = async () => {
    setLoading(true);
    await IntegrationService.connect("outlook");
    setStatus(IntegrationService.getStatus("outlook"));
    setLoading(false);
    showToast("Microsoft Outlook connected (Demo Mode).");
  };

  const handleDisconnect = async () => {
    await IntegrationService.disconnect("outlook");
    setStatus(IntegrationService.getStatus("outlook"));
    showToast("Microsoft Outlook disconnected.");
  };

  const handleSync = async () => {
    setLoading(true);
    await IntegrationService.sync("outlook");
    setStatus(IntegrationService.getStatus("outlook"));
    setLoading(false);
    showToast("Microsoft Outlook synced.");
  };
  ...
  return (
    <div className="workspace-container" style={{ maxWidth: 680 }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #0078d4, #004578)", borderRadius: 20, padding: "36px 32px", marginBottom: 28, color: "#fff" }}>
        <div style={{ fontSize: 48, marginBottom: 10 }}>📧</div>
        <h1 style={{ margin: 0, fontSize: 28, fontWeight: 800 }}>Microsoft Outlook</h1>
        <p style={{ margin: "8px 0 0", opacity: 0.9, fontSize: 15 }}>
          Integrate Voxora events and deadlines with Microsoft Outlook calendar and tasks.
        </p>
      </div>
      ...
      <div style={{ background: "var(--bg-card,#fff)", border: "1.5px solid var(--border,#e5e7eb)", borderRadius: 16, padding: "24px" }}>
        ...
        <div style={{ background: "#f0f9ff", border: "1px solid #bae6fd", borderRadius: 10, padding: "12px 16px", marginBottom: 20 }}>
          <p style={{ margin: 0, fontSize: 13, color: "#0369a1" }}>
            <strong>Demo Mode:</strong> Simulate connecting without real credentials. Add Microsoft Azure AD OAuth credentials in Integration Settings to go live.
          </p>
        </div>

        <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>What gets synced:</h3>
        {["Project milestones → Outlook Events", "Business deadlines → Tasks", "Meeting notes → Calendar entries", "Team goals → Shared calendars"].map(f => (
