...
export default function IntGitHub({ setWorkspace }: Props) {
  const [status, setStatus] = useState(IntegrationService.getStatus("github"));
  ...
  const handleConnect = async () => {
    ...
    setStatus(IntegrationService.getStatus("github"));
    setLoading(false);
    showToast("GitHub connected (Demo Mode).");
  };

  const handleDisconnect = async () => {
    await IntegrationService.disconnect("github");
    setStatus(IntegrationService.getStatus("github"));
  ...
  const handleSync = async () => {
    setLoading(true);
    await IntegrationService.sync("github");
    setStatus(IntegrationService.getStatus("github"));
    setLoading(false);
    showToast("GitHub repository synced.");
  };
  ...
  return (
    <div className="workspace-container" style={{ maxWidth: 680 }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #161b22, #24292f)", borderRadius: 20, padding: "36px 32px", marginBottom: 28, color: "#fff" }}>
        <div style={{ fontSize: 48, marginBottom: 10 }}>🐙</div>
        <h1 style={{ margin: 0, fontSize: 28, fontWeight: 800 }}>GitHub</h1>
        <p style={{ margin: "8px 0 0", opacity: 0.9, fontSize: 15 }}>
          Link GitHub repositories to Voxora projects, track commits, and monitor repo health.
        </p>
      </div>
      ...
      <div style={{ background: "var(--bg-card,#fff)", border: "1.5px solid var(--border,#e5e7eb)", borderRadius: 16, padding: "24px" }}>
        ...
        <div style={{ background: "#f0f9ff", border: "1px solid #bae6fd", borderRadius: 10, padding: "12px 16px", marginBottom: 20 }}>
          <p style={{ margin: 0, fontSize: 13, color: "#0369a1" }}>
            <strong>Demo Mode:</strong> Simulate connecting without real credentials. Add your GitHub OAuth App credentials in Integration Settings to go live.
          </p>
        </div>

        <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>What gets synced:</h3>
        {[
          "Link repos to Voxora projects",
          "Track commits and pull requests",
          "Monitor open issues count",
          "Trigger automations on pushes",
          "Display repo health in dashboards",
        ].map(f => (
