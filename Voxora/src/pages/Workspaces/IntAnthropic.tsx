// ── V4.8 Anthropic Claude Integration ────────────────────────────────────────
...
export default function IntAnthropic({ setWorkspace }: Props) {
  const { addActivity } = useActivity();
  const { showToast } = useToast();

  const [apiKey, setApiKey] = useState(() => localStorage.getItem("voxora-anthropic-key-hint") || "");
  const [model, setModel] = useState(() => localStorage.getItem("voxora-anthropic-model") || "claude-3-5-sonnet");
  const [status, setStatus] = useState<"idle" | "testing" | "ok" | "fail">("idle");
  const [enabled, setEnabled] = useState(() => localStorage.getItem("voxora-int-anthropic-enabled") !== "false");

  const save = () => {
    if (!apiKey.trim()) { showToast("Enter your Anthropic API key.", "error"); return; }
    localStorage.setItem("voxora-anthropic-key-hint", apiKey.slice(0, 8) + "…");
    localStorage.setItem("voxora-anthropic-model", model);
    localStorage.setItem("voxora-int-anthropic-enabled", String(enabled));
    addActivity({ type: "integration_saved", title: "Anthropic Integration Saved", description: `Model: ${model}`, category: "Integrations", icon: "🤖" });
    showToast("✅ Anthropic Claude integration saved!");
  };
  ...
  const testConnection = () => {
    ...
    setTimeout(() => {
      setStatus("ok");
      showToast("✅ Anthropic connection test successful (simulated).");
    }, 1500);
  };
  ...
  return (
    <div className="workspace-container" style={{ maxWidth: 720 }}>
      ...
      <div style={{
        ...
        <h1 style={{ margin: 0, fontSize: 26, fontWeight: 800 }}>Anthropic Claude Integration</h1>
        <p style={{ margin: "8px 0 0", fontSize: 14, opacity: 0.9 }}>
          Connect Anthropic Claude to use Claude Opus, Sonnet, and Haiku across your Voxora workspaces.
        </p>
      </div>
      ...
      <div style={{
        ...
        <h3 style={{ margin: "0 0 8px", fontSize: 15, fontWeight: 700, color: "#92400e" }}>ℹ️ How to get your API key</h3>
        <ol style={{ margin: 0, paddingLeft: 20, fontSize: 13, color: "#b45309", lineHeight: 1.8 }}>
