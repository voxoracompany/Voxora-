...
Hint: ${s.hint}`;
  ...
  const exportPDF = () => {
    const content = SLIDES.map(s =>
      `<div class="slide"><h2>${escapeHtml(SLIDES.find(sl => sl.key === s.key)?.label)}</h2><div class="content">${escapeHtml(slides[s.key]).replace(/\n/g, "<br/>")}</div></div>`
    ).join("");
...
<div class="slide cover">
  ...
  <p style="font-size:20px;opacity:.8">${escapeHtml(stage)} · ${escapeHtml(industry)}</p>
